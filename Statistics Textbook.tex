...
\begin{minipage}[t]{.6\textwidth}
\begin{tabular}{>{\centering}p{2cm}|>{\raggedright}p{7.5cm}}
\textbf{Notations} & \textbf{What it means}\tabularnewline
\hline 
$A\cup B$ & Union of $A$ and $B$, $\left\{ x\mid x\in A\text{ \textbf{or} }x\in B\right\} $\tabularnewline
\hline 
$A\cap B$ & Intersection of $A$ and $B$, $\left\{ x\mid x\in A\text{ \textbf{and} }x\in B\right\} $\tabularnewline
\hline 
...
\hline 
$A'$ & Complement of set $A$, $\left\{ x\in\xi\mid x\not\in A\right\} $\tabularnewline
\hline 
...
If $A$ and $B$ are \textbf{independent events} then $\text{P}\left(A\cap B\right)=\text{P}\left(A\right)\times\text{P}\left(B\right)$.
\end{tcolorbox}
\end{minipage}}
...
Two ordinary fair dice are thrown, find the probability of rolling:

\begin{enumerate}[label=(\alph*)]

\item  the same number on both dice
...
Still not convinced? Imagine instead that there are $1000$ doors,
behind which one has a car. After the player picks a door, the host
opens the $998$ doors that the car is not behind. At this point,
the player knows that the door he chose either contains a prize (with
a probability of $0.1\%$), or more likely, nothing ($99.9\%$). Given
the opportunity, the player should definitely switch.
...
A random variable $X$ is called \textbf{discrete} if it has a \textbf{countable}\footnotemark number of possible values.
\end{tcolorbox}
\end{minipage}}
\footnotetext{One-to-one correspondence with the natural numbers}
\medskip
...
\item  the number of rotten apples in a basket of fruits,
...
\item E.g. $\text{P}\left(X=2\right)$ refers to the probability of obtaining
...
\begin{GC}{Finding Binomial Probabilities}
...
\begin{example}[Probabilities of a Biased Coin]
...
\text{P}\left(X\geq a\right) & =\text{P}\left(X>a\right)+\text{P}\left(X=a\right)=\text{P}\left(X> a\right)
\end{align*}
...
The alternative hypothesis, denoted by $H_{1}$, is used to contradict the null hypothesis.
...
Our light bulb manufacturer wants to calculate the probability of
...
\begin{GC}{Unbiased Estimates For Individual Data}
\begin{steps}[leftmargin=1.5cm]
...
\[
\overline{X}\sim\text{N}\left(80,\frac{97.454}{250}\right)\text{ approximately}
\]
...
Using GC, $p-\text{value}=0.00377\text{ (3 s.f.)}<0.05$
...
age, so the age is the \textbf{independent variable} and distance
...
diagram}. We usually place the independent variable on the horizontal
axis, and the dependent variable on the vertical axis.

\begin{minipage}[t]{.6\textwidth}

In the \textbf{Opening Problem}, the independent variable \textit{age}
...
\begin{GC}{Plotting a Scatter Diagram}
...
is
\[
\log y=0.78099+0.32718x\text{ (5 s.f.)}
\]
